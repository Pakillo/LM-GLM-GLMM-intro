...
## ---------------------------------------------------------------------------------------------------
estimate_contrasts(prop.glm)


## ----surv_read, echo = TRUE-------------------------------------------------------------------------
surv <- readr::read_csv('data/survival_expt.csv')
head(surv)


## ----surv_eda---------------------------------------------------------------------------------------
plot(Survived ~ Stress, data = surv)
...
## ----surv_visreg, echo=TRUE-------------------------------------------------------------------------
visreg(surv.glm, scale = 'response')


## ---------------------------------------------------------------------------------------------------
check_model(surv.glm)


## ----echo=TRUE--------------------------------------------------------------------------------------
simulateResiduals(surv.glm, plot = TRUE)


## ----echo = TRUE------------------------------------------------------------------------------------
simres <- simulateResiduals(surv.glm, refit = TRUE)
testDispersion(simres, plot = FALSE)


## ----echo = TRUE, message=FALSE---------------------------------------------------------------------
check_overdispersion(surv.glm)


## ----logreg_overdisp, echo=1------------------------------------------------------------------------
surv.overdisp <- glm(cbind(Survived, Died) ~ Stress, data = surv, family = quasibinomial)
summary(surv.overdisp)


## ----logreg_overdisp2, echo=TRUE--------------------------------------------------------------------
parameters(surv.overdisp)
...
visreg(surv.glm, scale = 'response', main = 'Binomial')


## ----overdisp_plot2, echo=FALSE, out.width='100%'---------------------------------------------------
visreg(surv.overdisp, scale = 'response', main = 'Quasibinomial')
